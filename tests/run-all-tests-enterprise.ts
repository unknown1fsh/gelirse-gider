...
// Bu script tüm API endpoint'lerini Enterprise kullanıcı ile test eder.
async function main() {
  console.log('🧪 TÜM API ENDPOINT TESTLERİ (ENTERPRISE KULLANICI)\n')
  ...
  // Enterprise test kullanıcısı oluştur
  console.log('1️⃣  Enterprise test kullanıcısı oluşturuluyor...')

  const testEmail = `test-enterprise-${Date.now()}@test.com`
  const testPassword = 'Enterprise123456'
  let authToken = ''

  // Eski enterprise test kullanıcılarını temizle
  await prisma.user.deleteMany({
    where: {
      email: {
        startsWith: 'test-enterprise-',
      },
    },
  })
  ...
  console.log('   ✅ Register başarılı (Enterprise plan)')

  // Kullanıcının planını ENTERPRISE olarak ayarla
  ...
  console.log('   ✅ Plan enterprise olarak ayarlandı')
  ...
  console.log('   ✅ Login başarılı (Enterprise token alındı)')
  console.log()
  ...
  console.log("2️⃣  Tüm endpoint'leri test ediliyor (Enterprise token ile)...\n")
  ...
  console.log('='.repeat(100))
  console.log()
  console.log('📊 ENTERPRISE KULLANICI TEST SONUÇLARI:\n')
