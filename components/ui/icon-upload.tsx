...
const IconUpload = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & {
    onIconSelect?: (file: File) => void
    accept?: string
  }
>(({ className, onIconSelect, accept = ".ico,.png,.svg,.jpg,.jpeg,.gif,.bmp,.webp", ...props }, ref) => {
  const [isDragOver, setIsDragOver] = React.useState(false)

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files
    if (files && files.length > 0) {
      onIconSelect?.(files[0])
    }
  }
    ...
    >
      ...
      >
        <div className="flex flex-col items-center gap-2">
          <span>İkon dosyası seçin veya buraya sürükleyin</span>
